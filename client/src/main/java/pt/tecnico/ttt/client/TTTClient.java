		...
		try (Scanner scanner = new Scanner(System.in)) {
			...
			do {
				...
				do {
					...
					try {
						play_res = stub
								.play(PlayRequest.newBuilder().setRow(row).setColumn(column).setPlayer(player).build())
								.getRet();
				...
				CheckWinnerResponse win_res = stub.checkWinner(CheckWinnerRequest.newBuilder().build());
				winner = win_res.getRet();
				/* Select next player. */
				player = (player + 1) % 2;
				debug("player " + player);
